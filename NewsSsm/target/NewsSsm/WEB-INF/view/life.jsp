...
<div class="micronews-header-wrap">
  <div class="micronews-header w1000 layui-clear">
    ...
    <p class="nav">
      ...
      <a href="${ctx}/news/life.action" class="active">生活</a>
...
<script>
  layui.config({
    ...
    //动态显示新闻
    $.get("${ctx}/news/categoryPage.action?active="+active,function (res) {
      for(var i=0;i<res.length;i++){
        var imgage=encodeURIComponent(res[i].img)
        $('.list-item').append("<div class=\"item\">   \n" +
                "                <a target=\"_blank\" href=\"${ctx}/news/detailsPage.action?title="+res[i].title+"\">\n" +
                "                <img width=\"160px\" height=\"100px\" src=\"${ctx}/news/categoryNewsPhoto.action?address="+imgage+"\">\n" +
                "                </a>\n" +
                "                <div class=\"item-info\">\n" +
                "                  <h4><a target=\"_blank\" href=\"${ctx}/news/detailsPage.action?title="+res[i].title+"\">"+res[i].title+"</a></h4>\n" +
                "                  <div class=\"b-txt\">\n" +
    ...
    //这里是显示动态资讯
    $.get("${ctx}/news/categoryHotNews.action?active="+active,function (res) {
      for(var i=0;i<res.length;i++){
        $('.list-box').append("<li class=\"list\">\n" +
                "\n" +
                "                  <a target=\"_blank\" class='title' href=\"${ctx}/news/detailsPage.action?title="+res[i]+"\">"+res[i]+"</a>\n" +
                "\n" +
                "                </li>")
      }
    })
    ...
    //这里是给轮播图加链接
    $('.lun').attr("href","${ctx}/news/detailsPage.action?title=中国股市：为什么富人越炒越富，穷人越炒越穷？如果有10万资金，牢记犹太人“卖铜”思维，难怪这么赚钱")
    $('.lan').attr("href","${ctx}/news/detailsPage.action?title=特朗普真是“小天才”！在他的呼吁下，多家美企加大对中国的投资")
  });
</script>
<!--[if lt IE 9]>
